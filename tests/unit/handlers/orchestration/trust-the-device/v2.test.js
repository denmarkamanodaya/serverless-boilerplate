...
describe('Orchestration Handler', () => {
  ...
  beforeEach(async () => {
    ...
    trustTheDevice = require('../../../../../src/handlers/orchestration/trust-the-device/v2').handler;
  });

  test('POST /orchestration/trust-the-device. should error missing body', async () => {
    ...
    const response = await trustTheDevice(request);
    const data = JSON.parse(response.body);
    expect(data.status).toBe(httpStatus.BAD_REQUEST);
    expect(data.data.message).toBe('"deviceId" is required');
  });

  test('POST /orchestration/trust-the-device. should error DEVICE_NOT_OWNED', async () => {
    kmsMock.on(SignCommand).resolves({
      Signature: 'signature',
    });
    kmsMock.on(VerifyCommand).resolves({
  ...
  test('POST /orchestration/trust-the-device. should error 500 onboarding-service', async () => {
  ...
  test('POST /orchestration/trust-the-device. should 200/ DEVICE_HAS_BEEN_TRUSTED', async () => {
    ...
    expect(data.data).toBe('DEVICE_HAS_BEEN_TRUSTED');
  });
});
