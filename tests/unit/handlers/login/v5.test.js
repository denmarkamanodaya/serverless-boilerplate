...
describe('handler-login-v5', () => {
  ...
  beforeEach(() => {
    ...
    handler = require('../../../../src/handlers/login/v5').handler;
  });

  test('should throw an error when user does not exist in users table', async () => {
    eventData.body = JSON.stringify({ username: 'notvalid', password: 'password', deviceId: 'test' });
  ...
  test('should throw an error when user is not active', async () => {
    ...
    eventData.body = JSON.stringify({ username: 'username', password: 'password', deviceId: 'test' });
  ...
  test('should migrate the user when customer id is not equal to membership id in user db', async () => {
    ...
    eventData.body = JSON.stringify({
      ...
      deviceId: '56bdbeb7d526b52e09f1ec324abe1500a2dbb066eb1eb8c9699789e1960c6a08',
    });
