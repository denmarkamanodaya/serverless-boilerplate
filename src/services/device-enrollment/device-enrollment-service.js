...
const { sign, decode, verify } = require('../../utils/helpers/jwt-helperv2');
...
class DeviceEnrollmentService {
  async checkIfTrusted(data) {
    try {
      // Get mobile# from the current session
      const bearer = data.headers.authorization.split(' ')[1];
      const isValid = await verify(bearer);
      if (!isValid) {
        return this.error({
          message: jwtResponseCodes.JWT_INVALID.value,
          responseCode: jwtResponseCodes.JWT_INVALID.value,
      ...
      if (device.accessCode === null) {
        const secret = await this.generateAccessCode({ deviceId, mobileNumber, authServiceId });
  ...
  async otpChallengeGenerate(data) {
    try {
      const enrollmentToken = data.headers['x-device-enrollment-token'];
      const isValid = await verify(enrollmentToken);
      if (!isValid) {
        return this.error({
          message: jwtResponseCodes.JWT_INVALID.value,
          responseCode: jwtResponseCodes.JWT_INVALID.value,
  ...
  async otpChallengeValidate(data) {
    try {
  ...
  async confirmEnrollment(data) {
    try {
      ...
      const secret = await this.getSecret({ mobileNumber, deviceId });
      const isOtpAuthValid = authenticator.check(inAppOtp, secret, totpOptions);
      ...
      const deviceAssociation = await this.checkDeviceAssociation({
        username: mobileNumber,
        mobileInstanceId: deviceId,
      });
  ...
  async checkDeviceAssociation(data) {
    try {
  ...
  async validateTOTP(data) {
    try {
      ...
      console.log(inAppOtp, secret, totpOptions, isOtpAuthValid, deviceId, mobileNumber);
